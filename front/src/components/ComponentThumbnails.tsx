    ...
    <>
      ...
      {(
        <div className="relative flex-shrink-0" ref={mcpRef}>
          ...
          {/* MCP 列表弹窗 */}
          {showMCPList && (
            <div className="absolute bottom-full left-0 mb-2 w-80 bg-white dark:bg-[#141414] rounded-lg shadow-xl border border-gray-200 dark:border-[rgba(0,212,170,0.15)] z-50 max-h-96 overflow-y-auto">
              <div className="p-3 border-b border-gray-200 dark:border-[rgba(0,212,170,0.10)]">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Plug className="w-4 h-4 text-emerald-600 dark:text-[#00d4aa]" />
                    <h3 className="text-sm font-semibold text-gray-900 dark:text-[#e0e0e0]">MCP 服务器</h3>
                    <span className="text-xs text-gray-500 dark:text-[#888]">
                      ({connectedCount}/{mcpServers.length}已连接)
                    </span>
                    {selectedMcpServerIds.size > 0 && (
                      <span className="text-xs text-emerald-600 dark:text-[#00d4aa] font-medium">
                  ...
                  <button
                    ...
                    className="text-gray-400 hover:text-gray-600 dark:text-[#666] dark:hover:text-[#aaa]"
                    title="关闭"
                  >
                    <X className="w-4 h-4" />
                  </button>
                  ...
                  availableMcpServers.map((server) => {
                    ...
                    return (
                      <div
                        ...
                        className={`
                          ...
                          ${isConnecting
                            ...
                            : !isConnected 
                              ? 'hover:bg-yellow-50 dark:hover:bg-yellow-900/20 border border-dashed border-gray-300 dark:border-[#333]'
                              : isSelected
                      ...
                      >
                          ...
                          <div className="relative">
                            {isConnecting ? (
                              ...
                              <>
                                ...
                                {isConnected && (
                                  <span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-emerald-500 dark:bg-[#00d4aa] rounded-full border border-white dark:border-[#1a1a1a]"></span>
                                )}
                              </>
                            )}
                          </div>
                        ...
                        {isSelected && (
                          <div className="w-2 h-2 rounded-full bg-emerald-600 dark:bg-[#00d4aa] ml-2 flex-shrink-0"></div>
                        )}
                      </div>
                    );
                  })
          ...
          {showWorkflowList && (
            <div className="absolute bottom-full left-0 mb-2 w-80 bg-white dark:bg-[#141414] rounded-lg shadow-xl border border-gray-200 dark:border-[rgba(0,212,170,0.15)] z-50 max-h-96 overflow-y-auto">
              <div className="p-3 border-b border-gray-200 dark:border-[rgba(0,212,170,0.10)]">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <WorkflowIcon className="w-4 h-4 text-emerald-600 dark:text-[#00d4aa]" />
                    <h3 className="text-sm font-semibold text-gray-900 dark:text-[#e0e0e0]">工作流</h3>
                    {selectedWorkflowIds.size > 0 && (
                      <span className="text-xs text-emerald-600 dark:text-[#00d4aa] font-medium">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          ...
                          <span className="text-sm text-gray-900 dark:text-[#e0e0e0] truncate">
                            {workflow.name}
                          </span>
                        </div>
                        {isSelected && (
          ...
          {showSkillPackList && (
            <div className="absolute bottom-full left-0 mb-2 w-80 bg-white dark:bg-[#141414] rounded-lg shadow-xl border border-gray-200 dark:border-[rgba(0,212,170,0.15)] z-50 max-h-96 overflow-y-auto">
              <div className="p-3 border-b border-gray-200 dark:border-[rgba(0,212,170,0.10)]">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Package className="w-4 h-4 text-emerald-600 dark:text-[#00d4aa]" />
                    <h3 className="text-sm font-semibold text-gray-900 dark:text-[#e0e0e0]">技能包</h3>
                    {selectedSkillPackIds.size > 0 && (
                      <span className="text-xs text-emerald-600 dark:text-[#00d4aa] font-medium">
      ...
      {/* 附件上传按钮 - Tag样式 */}
      <label className="cursor-pointer flex-shrink-0 flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[11px] transition-all text-gray-400 dark:text-[#555] hover:text-gray-600 dark:hover:text-[#888]" title="上传图片或视频">
