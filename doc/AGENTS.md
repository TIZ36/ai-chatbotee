...
## 构建、测试与开发命令
- 前端：`npm install`，`npm run dev`（5177），`npm run build`（Vite + Electron TS），`npm run preview`。
...
- 最低手动检查：启动后端(3002) + Vite(5177) + Electron，验证工作流创建、MCP/LLM 配置保存与加载、终端命令执行，并在 PR 中注明回归情况。
...
- 默认端口：前端 5177、后端 3002、MySQL 3306、Redis 6379；如调整请同步文档。
