...
func (fs *FanoutService) PublishRootMessage(ctx context.Context, req *PublishRootRequest) error {
	redis := fs.cfg.Pools.GetRedis()
...
func (fs *FanoutService) PublishReplyMessage(ctx context.Context, req *PublishReplyRequest) error {
	redis := fs.cfg.Pools.GetRedis()
...
// PublishReplyRequest represents a request to publish reply message
...
func isAI(id string) bool {
	return len(id) > 0 && (id[0] == 'a' || id[:3] == "ai_")
}
