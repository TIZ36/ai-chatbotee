...
func NewCoreService(cfg Config, hbaseRepo repository.HBaseRepository) *CoreService {
...
func (s *CoreService) handleAIAgents(ctx context.Context, chatKey, msgID string, mentions, aiAgents []string, req *SendChatMessageRequest) {
	...
	for _, mention := range mentions {
		if isAI(mention) && contains(aiAgents, mention) {
			s.cfg.Logger.Info("AI agent mentioned in chat",
				"agent_id", mention,
				"chat_key", chatKey,
				"msg_id", msgID)
			// TODO: 调用AiAgentActor.OnChatMessage
		}
	}
	...
	for _, agentID := range aiAgents {
		if !contains(mentions, agentID) {
			s.cfg.Logger.Info("Updating AI memory for chat",
				"agent_id", agentID,
				"chat_key", chatKey)
			// TODO: 异步更新AI记忆
		}
