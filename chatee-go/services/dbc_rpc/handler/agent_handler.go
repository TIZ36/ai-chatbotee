...
func NewAgentHandler(repo repository.AgentRepository, logger log.Logger) *AgentHandler {
	return &AgentHandler{
		repo:   repo,
		logger: logger,
	}
...
func (h *AgentHandler) GetAgentsByUser(ctx context.Context, req *dbc.GetAgentsByUserRequest) (*dbc.GetAgentsByUserResponse, error) {
	...
	if err != nil {
		h.logger.Error("Failed to get agents by user", log.Err(err))
...
func (h *AgentHandler) UpdateAgent(ctx context.Context, req *dbc.UpdateAgentRequest) (*dbc.Agent, error) {
	...
	if err != nil {
		...
		h.logger.Error("Failed to get agent for update", log.Err(err))
		return nil, status.Errorf(codes.Internal, "failed to get agent: %v", err)
	}
...
func (h *AgentHandler) toProtoAgent(agent *repository.Agent) *dbc.Agent {
	description := ""
